...
def run_dorado(sample, input_dir, output_dir):
    ...
    cmd = f"{dorado_path} basecaller {model_path} {input_file} --emit-fastq > {output_file} -I 4G -x cuda:1 --batchsize 64"
    os.system(cmd)

for sample in samples_bov:
    run_dorado(sample, pod5_out_bov, pod5_out_bov)

for sample in samples_mid:
    run_dorado(sample, pod5_out_mid, pod5_out_mid)

for sample in samples_onder:
    run_dorado(sample, pod5_out_onder, pod5_out_onder)

for sample in samples_pac:
    run_dorado(sample, pod5_out_pac, pod5_out_pac)
