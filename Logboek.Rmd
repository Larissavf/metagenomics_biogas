...
Geinstalleer met : pip install pod5

Dit is om dorado basecaller beter te runnen.
Hier word fast5 omgeschreven naar pod5 formaat.

### Dorado.py 

Waarom Dorado basecaller.
De eerder verkregen data is van Guppy basecaller dat relatief oud is.
Dorado doet het om schrijven van fast5 naar fastq nauwkeuriger.

Er is gebruik gemaakt van dorado-0.5.3-linux-x64

Losse script om dorado werkend te krijgen.

Na verschillende errors is het niet gelukt om dorado in de pipline te krijgen.
- CUDA error: out of memory

Daarom heb ik verschillende dingen geprobeert:
- batch size aanpassen
- Door dorado voorgesteld oplossing
- Eigen script

Met --batchsize in de dorado command is er 64 meegegeven wat het laagst is.
Met -I 4G Stelt een interne buffer groote aan. 4G is het laagste. 
Om zo de memery min mogelijk te belasten.
Met -x cuda:0 aangegeven welke cuda er gebruikt word om zo eventueel het probleem te verhelpen

Cuda 0 lijkt meer memory te hebben.
Dit is bekenen met behulp van: nvidia-smi -l 1 en nvitop

Ook is er gebruik gemaakt van de minst vereisende model de fast in plaats van de sup.
Wat niet optimaal is.


Als de snake rule gebruikt dan maakte die elk sample tussen 0 en 98 procent af.  
Om vervolgens de file te verwijderen of een 0 byte fastQ te maken  
Dan gaf die een bash error. 
Als er vervolgen sde  regel uitvoerde wat de error gaf deed die het los wel. 
Daarom is er een simpel python script geschreven dat elk command los uitvoerd. 
Dit leek het in eerste instantie ook niet te doen. 

Toen is er verder gekeken naar de CUDA error: out of memory. 
Eerst is er gekeken naar de graphice kaart of daar veel procces bezig waren. 
Dit bleek niet het geval. 
Er is eerst gebruik gemaakt van meerdere cores maar ook van 1.  
Om zo te kijken of dat verschil maakt voor de memory. 

Toen is er gebruikt gemaakt van export 'PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:(32, 64, 128, 512) 
Hier heb ik verschillende de split gebruikt om te kijken of dat het probleem zou verhelpen. 
Het leek het wel beter te doen maar nog steeds niet goed genoeg. 
Elk sample werd nu wel gerund maar ook niet tot 100%. 

Toen is er gebruik gemaakt van tourch dat met snakemake werkt. 
Hier heb is dit command gebruikt torch.cuda.empty_cache() . 
Hiervoor moet er wel eerst tourch worden geinstalleerd.  
Dit is gedaan met: pip install torch.

Dit staat script om de cache te leggen. 
Dit verhielp het probleem ook niet. 

Toen heb is er verder gezocht en gebruik gemaakt van TORCH_USE_CUDA_DSA=1 
Wat het probleem ook niet verholp. 

Nadat is het internet weer raad gepleegd. 

Voor dat dorado gebruikt word export CUDA_LAUNCH_BLOCKING=1 uit te voeren. 
Om de settings van cuda aan te passen.

Toen is er ook gebruik gemaakt van  export CUDA_VISIBLE_DEVICES=1

Na het zo goed als opgegeven te hebben en bij het schrijven van dit logboek om zo alle errors te reproduceren det het script het wel.
Eerst als ik het script runde gaf die geen error maar 0 byte fastq files.
Maar op het randje van opgeven deed die het wel.

Het nadeel van het script is dat je niet gebruik kan maken van meerdere cores waardoor dat erg lang duurt.
